...
class RatingComponent {
  ...
  RatingComponent(this._scope, this._element, this._ngModel, this._ratingConfig) {
    maxRange = _element.attributes.containsKey('max') ? _scope.parentScope.eval(_element.attributes['max']) : _ratingConfig.max;
    stateOn = _element.attributes.containsKey('state-on') ? _scope.parentScope.eval(_element.attributes['state-on']) : _ratingConfig.stateOn;
    stateOff = _element.attributes.containsKey('state-off') ? _scope.parentScope.eval(_element.attributes['state-off']) : _ratingConfig.stateOff;
    
    _ngModel.render = (value) {
      val = _ngModel.viewValue;
    };
    range = _buildTemplateObjects(_element.attributes.containsKey('rating-states') ? _scope.parentScope.eval(_element.attributes['rating-states']) : new List(maxRange));
  }
